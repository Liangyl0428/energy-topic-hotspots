...
“任务单元”就是一个定义明确、单独统计证据的应用方向，例如“虚拟电厂资源聚合与调度”。这里评估了四个方向，各自的结论如下：

| 应用方向 | 只用明确相关文献计算 | 再加入范围待判断的文献 | 如何理解 |
| --- | --- | --- | --- |
| 虚拟电厂 | 达标 | 达标 | 两种纳入范围下都满足条件，可列为跟踪方向 |
| 电力大模型辅助决策与运维 | 达标 | 达标 | 两种纳入范围下都满足条件，可列为跟踪方向 |
| 空气源热泵 | 达标 | 未达标 | 结论依赖哪些文献被纳入；先核实待判断记录，再决定是否推荐 |
| 电力知识图谱与知识问答 | 未达标 | 未达标 | 目前证据未满足全部条件，暂不推荐为潜在热点，保留观察 |

这两种纳入范围在技术上称为**严格口径**和**扩展口径**。“双口径支持”就是两次计算都达标；每次计算都同时使用论文、专利和政策证据。“达标”指满足全部入选条件，包括至少2组明确政策支持。

空气源热泵的1—8月专利/论文相对份额，从只用明确相关记录时的 **1.85** 降到加入待判断记录后的 **1.19**，低于 **1.25** 的门槛。因此它是“有条件跟踪”。知识图谱则只确认 **1组**明确政策，未达到至少2组的要求，因此是“保留观察”。

“可跟踪”表示有当前证据支持持续关注，并不代表已经证明技术领先或未来一定成功。这四个方向也不代表对全部750个主题的潜在机会做了穷尽评估。
